...
public class ThunderstormManager {
    ...
    public void tick() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            if (holding) {
                ...
                maybeStrike(player);
                // FIX (previously known bug): storm ambience only played ONCE at the exact spot the
                // player was standing when they equipped the trident, never re-anchoring as they
                // moved. Now re-anchored to the wielder's LIVE location every second, plus the
                // previously-requested particle ring marking the exact 25-block radius.
                tickAmbience(player);
    ...
    /** Re-anchors the storm's ambient sound and draws the radius-marking particle ring at the
     *  wielder's CURRENT location every second - fixes the storm not following the wielder. */
    private void tickAmbience(Player player) {
        Location center = player.getLocation();
        player.getWorld().playSound(center, Sound.WEATHER_RAIN, 0.4f, 0.8f);

        int points = 48;
        for (int i = 0; i < points; i++) {
            double angle = 2 * Math.PI * i / points;
            double x = center.getX() + TridentType.STORM_RADIUS * Math.cos(angle);
            double z = center.getZ() + TridentType.STORM_RADIUS * Math.sin(angle);
            Location ringPoint = new Location(player.getWorld(), x, center.getY(), z);
            player.getWorld().spawnParticle(Particle.ELECTRIC_SPARK, ringPoint, 1, 0, 0.3, 0, 0);
        }
    }
    ...
    public void strike(LivingEntity target, double damage, int stunTicks, boolean placeMagma, boolean forceOverride) {
        ...
        // FIX (previously known bug): never reset noDamageTicks before applying damage, so rapid
        // strikes (especially the 0.5s-interval bolt chain) could land inside the target's ~0.5s
        // hurt-invulnerability window and get silently absorbed with no damage and no visible tick.
